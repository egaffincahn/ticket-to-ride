...
for g = 1:generations
    ...
    % not everyone plays each generation
...
%     matchups = reshape(order, [individuals/players,players]);
    for i = 1:individuals/players % scroll through each matchup
        gameplayers = randsample(1:length(individuals), players * 2, false);
%         gameplayers = matchups(i,:); % choose first 3 of random perm
        [gamepointsA, winnerA] = playgame(G, strategies(gameplayers(1:3)), cards, info);
        [gamepointsB, winnerB] = playgame(G, strategies(gameplayers(4:6)), cards, info);
        losers = [losers, gameplayers(1:players ~= winner)];
